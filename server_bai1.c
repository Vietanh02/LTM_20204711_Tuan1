...
int main(int argc, char *argv[])
{
    ...
    addr.sin_addr.s_addr = INADDR_ANY; // VA has repare this line
    addr.sin_port = htons(atoi(argv[1]));

    addr.sin_zero[8] = '\0';
    // Viet Anh has written the following line----------------------------------
    int option = 1;
    int status = setsockopt(serverSocket, SOL_SOCKET, SO_REUSEADDR, &option, sizeof(option));
    if (status < 0)
    {
        printf("error");
        exit(1);
